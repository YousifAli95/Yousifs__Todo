"use strict";
class listItem {
    constructor(nummer, content, isChecked) {
        this.nummer = nummer;
...
class saveListClass {
    constructor(itemArray, nonCheckedArray, checkedArray, nummer) {
...
(function (flik) {
    flik[flik["all"] = 0] = "all";
    flik[flik["nonChecked"] = 1] = "nonChecked";
    flik[flik["checked"] = 2] = "checked";
})(flik || (flik = {}));
//****Get selectors***********************************/
...
const saveInputText = (document.querySelector("#save-input-box"));
const clearBtn = document.querySelector("#clear-bottom");
const clearListBtn = document.querySelector("#clear-list");
const actionBar = document.querySelector(".action-bar");
const actionBarSpan = (document.querySelector(".action-bar-span"));
//****Variables*****************************************/
var number = 0;
var ItemArray = [];
var checkedArray = [];
...
window.onload = function () {
    ...
    if (localStorage.getItem("savedListsObject") !== null) {
        savedListObject = JSON.parse(localStorage.getItem("savedListsObject") || "");
    }
    let savedLists = document.querySelector("#savedLists");
    ...
    if (currentList !== "") {
        ...
        const div = document.querySelector(".current-list-box");
    ...
    if (currentFlik === flik.checked) {
        const checked = document.querySelector("#checked-items");
        flikFunction(checked, currentFlik);
    }
    else if (currentFlik === flik.nonChecked) {
        const checked = (document.querySelector("#non-checked-items"));
        flikFunction(checked, currentFlik);
    }
...
function upArrowFunction(itemNumber) {
    if (currentFlik === flik.checked) {
...
function downArrowFunction(itemNumber) {
    if (currentFlik === flik.checked) {
        const indexCheckedArray = checkedArray.findIndex((object) => {
            return object.nummer === itemNumber;
        });
...
function tick(siffra) {
    const index = ItemArray.findIndex((object) => {
        return object.nummer === siffra;
    });
    let spannTick = (document.querySelector(`.span-list${siffra}`));
...
function tickbefore(tickArray) {
    for (let index = 0; index < tickArray.length; index++) {
        let spannTick = (document.querySelector(`.span-list${tickArray[index].nummer}`));
...
function saveList() {
    ...
    if (currentList !== "") {
        savedListObject[currentList].itemArray = JSON.parse(JSON.stringify(ItemArray));
        savedListObject[currentList].checkedArray = JSON.parse(JSON.stringify(checkedArray));
        savedListObject[currentList].nonCheckedArray = JSON.parse(JSON.stringify(nonCheckedArray));
        savedListObject[currentList].nummer = number;
    }
    localStorage.setItem("currentList", currentList);
    localStorage.setItem("savedListsObject", "" + JSON.stringify(savedListObject));
}
function listHover(listItem, siffra) {
...
function listOut(listItem, siffra) {
    let Check = document.getElementById(`check${siffra}`);
    const index = ItemArray.findIndex((object) => {
        return object.nummer === siffra;
    });
...
function flikFunction(html, flikChoice) {
    const all = document.querySelector("#all");
    const checked = document.querySelector("#checked-items");
    const nonChecked = (document.querySelector("#non-checked-items"));
...
function actionBarFunction(message, backgroundColor, textColor = "black") {
...
function penFunction(nummer) {
    const index = ItemArray.findIndex((object) => {
        return object.nummer === nummer;
    });
    const inputNummer = "input" + nummer;
    let liList = document.querySelector(`#li-list${nummer}`);
...
`;
    const input = document.querySelector(`#${inputNummer}`);
    input.focus();
}
function Edit(nummer) {
    const inputNummer = "input" + nummer;
    const newValue = document.querySelector(`#${inputNummer}`)
        .value;
    const index = ItemArray.findIndex((object) => {
        return object.nummer === nummer;
    });
...
function noSaveFunction() {
    const saveRow = document.querySelector(".save-row");
    saveRow.style.maxHeight = "0em";
}
function noLoadFunction() {
    const loadRow = document.querySelector(".load-row");
    loadRow.style.maxHeight = "0em";
    const btn = document.querySelector(".margin-div");
...
function selectFunction(value) {
    currentList = value;
    ItemArray = JSON.parse(JSON.stringify(savedListObject[currentList].itemArray));
    checkedArray = JSON.parse(JSON.stringify(savedListObject[currentList].checkedArray));
    nonCheckedArray = JSON.parse(JSON.stringify(savedListObject[currentList].nonCheckedArray));
    number = savedListObject[currentList].nummer;
    flikUpdateList();
    saveList();
...
function saveListbtn() {
    const listName = document.querySelector(`#save-input-box`)
        .value;
    savedListObject[listName] = new saveListClass(JSON.parse(JSON.stringify(ItemArray)), JSON.parse(JSON.stringify(nonCheckedArray)), JSON.parse(JSON.stringify(checkedArray)), number);
